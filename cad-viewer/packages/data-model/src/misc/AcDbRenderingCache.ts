...
export class AcDbRenderingCache {
  ...
  private evictOldest() {
    ...
    if (!template) return
    if (this.isCompactedTemplate(template)) {
  ...
  /**
   * Duck-typed check for three-renderer's compacted group state without
   * importing the renderer package.
   */
  private isCompactedTemplate(template: AcGiEntity) {
    return (
      (template as unknown as { isCompacted?: unknown }).isCompacted === true
    )
  }
